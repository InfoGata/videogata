...
/**
 * Hook to get just the player HTML size for display purposes
 */
export function usePluginPlayerSize(pluginId: string): number {
  const isNative = isFilesystemStorage();
  const [size, setSize] = React.useState(0);
  ...
  React.useEffect(() => {
    if (isNative && pluginId) {
      loadPluginField(pluginId, "playerHtml").then((html) => {
        if (html) {
          setSize(new Blob([html]).size);
        }
      });
    } else if (dexiePlugin?.playerHtml) {
      setSize(new Blob([dexiePlugin.playerHtml]).size);
    }
  }, [isNative, pluginId, dexiePlugin]);
