 ...
 *   player.html     - Player page (optional)
 */

const PLUGINS_DIR = "plugins";

type PluginLargeField = "script" | "optionsHtml" | "playerHtml";
 ...
 */
export interface PluginMetadata extends Omit<PluginInfo, "script" | "optionsHtml" | "playerHtml"> {
  script: string; // Empty string on native - actual content in filesystem
  hasScript?: boolean;
  hasOptionsHtml?: boolean;
  hasPlayerHtml?: boolean;
}
...
export async function savePlugin(plugin: PluginInfo): Promise<void> {
  ...
  if (plugin.playerHtml) {
    await writePluginField(pluginId, "playerHtml", plugin.playerHtml);
  }
  ...
  const metadata: PluginMetadata = {
    ...
    hasPlayerHtml: !!plugin.playerHtml,
  };
  delete (metadata as any).optionsHtml;
  delete (metadata as any).playerHtml;
...
export async function loadPlugin(pluginId: string): Promise<PluginInfo | undefined> {
  ...
  if (nativeMetadata.hasPlayerHtml) {
    const playerHtml = await readPluginField(pluginId, "playerHtml");
    if (playerHtml) plugin.playerHtml = playerHtml;
  }
...
/**
 * Load a single field from a plugin (optimized for options/player pages).
 */
    ...
    for (const pluginId of pluginIds) {
      try {
        ...
        if (plugin.script && plugin.script.length > 0 && !metadata.hasScript) {
          ...
          delete (updatedMetadata as any).playerHtml;
