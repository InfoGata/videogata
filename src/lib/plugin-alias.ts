...
/**
 * Plugin ids are stable and shareable but unreadable (`/s/a7f3e9b2c1d4/videos/x`).
 * An alias is a short readable name assigned locally, so no plugin author can
 * squat a name and two plugins for the same site can coexist (`youtube`,
 * `youtube-2`). Ids remain the identity everywhere in the app; the alias only
 ...
 * that into the url defeats the point. Drop the "plugin for" lead-in, cut
 * anything from "plugin" onwards, and keep what names the site.
 */
export const aliasFromName = (name: string): string => {
  const stripped = name
    .replace(/^\s*plugins?\s+for\s+/i, "")
    .replace(/\bplugins?\b.*$/i, "");
...
// is primed before the router renders and refreshed whenever plugin frames are
// published (see src/router.tsx and src/contexts/PluginsContext.tsx).
